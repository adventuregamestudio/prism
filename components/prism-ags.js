Prism.languages.ags = {
	...
	/*
	  ...
	  --------
	  "AGS_MAX_HOTSPOTS" "AGS_MAX_INV_ITEMS" "AGS_MAX_OBJECTS" "AGS_MAX_REGIONS" "AGS_NEW_STRINGS" "AGS_SUPPORTS_IFVER" "DEBUG" "LRPRECEDENCE" "MAX_LISTBOX_SAVED_GAMES" "NEW_DIALOGOPTS_API" "NEW_KEYINPUT_API" "PALETTE_SIZE" "SCRIPT_API_vXXX" "SCRIPT_COMPAT_vXXX" "SCR_NO_VALUE" "STRICT" "STRICT_AUDIO" "STRICT_STRINGS"
	*/
	'constant': /(?:AGS_(?:MAX_(?:(?:HOTSPOT|INV_ITEM|OBJECT|REGION)S)|NEW_STRINGS|SUPPORTS_IFVER)|DEBUG|LRPRECEDENCE|MAX_LISTBOX_SAVED_GAMES|NEW_(?:(?:DIALOGOPTS|KEYINPUT)_API)|PALETTE_SIZE|S(?:CR(?:IPT_(?:(?:API|COMPAT)_v\d+)|_NO_VALUE)|TRICT(?:_(?:AUDIO|STRINGS))?))/,
	...
	*/
	'enum-fixed': {
		pattern: /\be(?:A(?:lign(?:Bottom(?:Center|(?:Lef|Righ)t)|Center|Has(?:Bottom|HorCenter|Left|Right|Top|VerCenter)|Left|Middle(?:Center|(?:Lef|Righ)t)|None|Right|Top(?:Center|(?:Lef|Righ)t))|nywhere|udio(?:File(?:M(?:IDI|OD|P3)|OGG|VOC|WAV)|Priority(?:High|Low|Normal|Very(?:High|Low))))|B(?:ackwards|lock)|C(?:D(?:CloseTray|Eject|Get(?:CDDriveCount|(?:NumTrack|PlayingStatu)s)|IsDriverPresent|P(?:(?:ausePlayb|layTr)ack)|ResumePlayback|SelectActiveCDDrive)|ase(?:(?:Ins|S)ensitive))|Direction(?:Down(?:(?:Lef|Righ)t)?|Left|None|Right|Up(?:(?:Lef|Righ)t)?)|Event(?:AddInventory|EnterRoomBeforeFadein|G(?:UIMouse(?:Down|Up)|otScore)|L(?:eaveRoom|oseInventory)|RestoreGame)|F(?:ile(?:Append|Read|Write)|lip(?:Both|LeftToRight|UpsideDown)|orwards)|GUIPopup(?:Mo(?:dal|useYPos)|Normal|Persistent)|Input(?:Any|Keyboard|(?:Mous|Non)e)|Ke(?:epMoving|y(?:A(?:lt(?:(?:Lef|Righ)t)|mpersand|sterisk|t)|Back(?:Slash|space)|C(?:lose(?:Bracket|Parenthesis)|o(?:deMask|lon|mma)|trl(?:[A-Z]|(?:Lef|Righ)t))|D(?:elete|o(?:llar|ubleQuote|wnArrow))|E(?:nd|quals|scape|xclamationMark)|F(?:[1-9]|1[012]|orwardSlash)|GreaterThan|H(?:ash|ome|yphen)|Insert|Le(?:ftArrow|ssThan)|Mod(?:Alt(?:(?:Lef|Righ)t)?|C(?:aps|trl(?:(?:Lef|Righ)t)?)|Mask|Num|Shift(?:(?:Lef|Righ)t)?)|N(?:one|umPad5)|Open(?:Bracket|Parenthesis)|P(?:age(?:Down|Up)|er(?:cent|iod)|lus)|QuestionMark|R(?:eturn|ightArrow)|S(?:emiColon|hift(?:(?:Lef|Righ)t)|(?:ingleQuot|pac)e)|Tab|U(?:nderscore|pArrow)|[0-9A-Z]))|Lo(?:cation(?:Character|Hotspot|Nothing|Object)|g(?:Alert|Debug|Error|Fatal|Info|Warn))|Mouse(?:Left(?:Inv)?|Middle(?:Inv)?|Right(?:Inv)?|Wheel(?:(?:Nor|Sou)th))|No(?:Block|nSorted)|O(?:S(?:Android|DOS|FreeBSD|Linux|MacOS|PSP|W(?:eb|indows)|iOS)|nce|ptionO(?:ff(?:Forever)?|n))|R(?:epeat|ound(?:Down|Nearest|Up))|S(?:ay(?:Always|Never|UseOptionSetting)|eek(?:Begin|Current|End)|kip(?:AnyKey(?:OrMouseClick)?|ESCO(?:nly|rRightButton)|Key(?:(?:Mous(?:eTim)?|Tim)e)?|Mouse(?:Click|Time)?|None|ScriptOnly|Time)|orted|peech(?:FullScreen|Lucasarts|Sierra(?:WithBackground)?|TextOnly|Voice(?:AndText|Only))|topMoving)|Transition(?:Boxout|Crossfade|Dissolve|Fade|Instant)|VideoSkip(?:AnyKey(?:OrMouse)?|EscKey|NotAllowed)|WalkableAreas)\b/,
		alias: 'constant'
	},
	'enum-dynamic': {
